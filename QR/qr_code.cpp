...
QR_code::QR_code()
    : m_qr(nullptr), m_error_correction_level(qrcodegen::QrCode::Ecc::MEDIUM), m_border(4), m_decoration(default_decoration) {
}
...
std::string QR_code::get_svg() const {
	...
	auto viewbox_X = m_decoration.width * scale;
	auto viewbox_Y = m_decoration.height * scale;

	std::string viewbox   = F("0 0 {} {}", viewbox_X, viewbox_Y + viewbox_Y * 0.5);
	...
	std::string translate = "0.000000 1.500000";

//Lib is internally inconsistent, size is size_t but module is int
...
#pragma GCC diagnostic pop
	...
	replace(svg, "%offsetX%", "3");
	replace(svg, "%offsetY%", std::to_string(viewbox_Y));
