...
struct to_described_member {
	...
	object const&      obj;
	Ctx const&         ctx;
    std::size_t BJIntrusive_Count = 0;
...
value_to_impl(
    ...
    if( member_converter.BJIntrusive_Count != obj->size() )
    {
        BOOST_JSON_FAIL(BJIntrusive::error, error::size_mismatch);
        res = {boost::system::in_place_error, BJIntrusive::error};
        BJIntrusive_MESSAGE(fmt::format("Converted {} element out of {} in the json, while processing obj {} in {}", member_converter.BJIntrusive_Count,obj->size(), getTypeName<T>(), BJIntrusive::composePath()))
        return res;
    }
...
T initialize_variant(V&& v, mp11::mp_int<0>) {
	T t;
...
template <class T, class I, class V>
T initialize_variant(V&& v, mp11::mp_int<2>) {
	return T(variant2::in_place_index_t<I::value>(), std::move(v));
}

#ifndef BOOST_NO_CXX17_HDR_VARIANT
template <class T, class I, class V>
T initialize_variant(V&& v, mp11::mp_int<1>) {
...
mp11::mp_if<
    mp11::mp_valid<
        has_nonthrowing_user_conversion_to_impl, T>,
    system::result<T>>
value_to_impl(
...
template <class T, class Ctx>
T value_to_impl(no_conversion_tag, value_to_tag<T>, value const&, Ctx const&) {
	static_assert(
...
T value_to_impl(Impl impl, value_to_tag<T>, value const& jv, Ctx const& ctx) {
...
#ifndef BOOST_NO_CXX17_HDR_OPTIONAL
inline system::result<std::nullopt_t>
tag_invoke(
